...
void vm_run(u16_t mref)
{
  ...
  do
  {
    ...
    {
      stack_push(~NVM_IMMEDIATE_MASK & (arg0.w));
      pc_inc = 3;
      DEBUGF("sipush #" DBG16 "\n", stack_peek_int(0));
    }
    ...
    else if ((instr >= OP_IFEQ) && (instr <= OP_IF_ICMPLE))
    {
      ...
      {
      case OP_IF_ICMPEQ:
        DEBUGF("eq (%d %d)", tmp1, tmp2);
        tmp1 = (tmp1 == tmp2);
        break;
      case OP_IF_ICMPNE:
        DEBUGF("ne (%d %d)", tmp1, tmp2);
        tmp1 = (tmp1 != tmp2);
        break;
      case OP_IF_ICMPLT:
        DEBUGF("lt (%d %d)", tmp1, tmp2);
        tmp1 = (tmp1 < tmp2);
        break;
      case OP_IF_ICMPGE:
        DEBUGF("ge (%d %d)", tmp1, tmp2);
        tmp1 = (tmp1 >= tmp2);
        break;
      case OP_IF_ICMPGT:
        DEBUGF("gt (%d %d)", tmp1, tmp2);
        tmp1 = (tmp1 > tmp2);
        break;
      case OP_IF_ICMPLE:
        DEBUGF("le (%d %d)", tmp1, tmp2);
        tmp1 = (tmp1 <= tmp2);
        break;
      }
    ...
    else if ((instr >= OP_IADD) && (instr <= OP_IINC))
    {
      ...
      else if (((instr & 0x03) == 0x02) && (instr <= OP_FNEG))
      {
        ...
        else
        {
          ...
          {
          case OP_FADD:
            DEBUGF("fadd(%f,%f)", f1, f0);
            f1 += f0;
            break;
          case OP_FSUB:
            DEBUGF("fsub(%f,%f)", f1, f0);
            f1 -= f0;
            break;
          case OP_FMUL:
            DEBUGF("fmul(%f,%f)", f1, f0);
            f1 *= f0;
            break;
          case OP_FDIV:
            DEBUGF("fdiv(%f,%f)", f1, f0);
            if (!f0)
              error(ERROR_VM_DIVISION_BY_ZERO);
            f1 /= f0;
            break;
          case OP_IREM:
            DEBUGF("frem(%f,%f)", f1, f0);
      ...
      else
      {
        ...
        {
        case OP_IADD:
          DEBUGF("iadd(%d,%d)", tmp2, tmp1);
          tmp2 += tmp1;
          break;
        case OP_ISUB:
          DEBUGF("isub(%d,%d)", tmp2, tmp1);
          tmp2 -= tmp1;
          break;
        case OP_IMUL:
          ...
          break;
        case OP_IDIV:
          DEBUGF("idiv(%d,%d)", tmp2, tmp1);
          if (!tmp1)
            error(ERROR_VM_DIVISION_BY_ZERO);
          tmp2 /= tmp1;
          break;
        case OP_IREM:
          DEBUGF("irem(%d,%d)", tmp2, tmp1);
          tmp2 %= tmp1;
          break;
        case OP_ISHL:
          DEBUGF("ishl(%d,%d)", tmp2, tmp1);
          tmp2 <<= tmp1;
          break;
        case OP_ISHR:
          DEBUGF("ishr(%d,%d)", tmp2, tmp1);
          tmp2 >>= tmp1;
          break;
        case OP_IAND:
          DEBUGF("iand(%d,%d)", tmp2, tmp1);
          tmp2 &= tmp1;
          break;
        case OP_IOR:
          DEBUGF("ior(%d,%d)", tmp2, tmp1);
          tmp2 |= tmp1;
          break;
        case OP_IXOR:
          DEBUGF("ixor(%d,%d)", tmp2, tmp1);
          tmp2 ^= tmp1;
          break;
        case OP_IUSHR:
          DEBUGF("iushr(%d,%d)", tmp2, tmp1);
